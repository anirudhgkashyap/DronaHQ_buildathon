...
(function (A) {
  ...
  /* ---- mock helpers for campaign-detail-specific endpoints ---- */
  var _origHandle = A.mock && A.mock.handle;
  ...
  function stageBadge(s) {
    var map = {
      /* backend funnel stages (Prospect.stage) */
      discovered: "badge-draft", researched: "badge-draft", qualified: "badge-paused",
      contacted: "badge-paused", engaged: "badge-active", meeting: "badge-active", opportunity: "badge-active",
      /* legacy/demo aliases */
      new: "badge-draft", enrolled: "badge-draft", replied: "badge-active", meeting_booked: "badge-active", unsubscribed: "badge-draft"
    };
  ...
  async function loadCampaign() {
    try {
      ...
      el("cd-name").classList.remove("skel","skel--text","skel--wide");
      el("cd-icp").textContent = c.icp_summary || "";
      el("cd-icp").classList.remove("skel","skel--text");
      el("cd-badge").innerHTML = statusBadge(c.status);

      /* action buttons */
      var acts = "";
      acts += '<a class="btn btn--secondary btn--sm" href="campaign-new.html?id=' + encodeURIComponent(c.id) + '"><svg class="icon"><use href="#i-edit"/></svg>Edit</a>';
      ...
      ["pause","resume","archive"].forEach(function (action) {
        ...
        btn.addEventListener("click", async function () {
          btn.disabled = true;
          try {
            await A.api.campaignAction(c.id, action);
            location.reload();
          } catch (e) {
            alert(e.message);
            btn.disabled = false;
      ...
      ["m-prospects","m-contacted","m-replies","m-meetings","m-open-rate","m-reply-rate"].forEach(function (id) {
        el(id).classList.remove("skel","skel--text");
      });

      if (c.pending_approvals) {
        var badge = el("approvals-tab-count");
        badge.textContent = c.pending_approvals;
        badge.hidden = false;
      }
    } catch (e) {
      el("campaign-hd").innerHTML = '<p class="empty-state">' + e.message + '</p>';
    }
  }
  ...
  async function loadProspects() {
    ...
    try {
      ...
      data.items.forEach(function (p) {
        ...
        tr.innerHTML =
          '<td><div class="cell-name">' + esc(p.name) + '</div><div class="cell-sub">' + esc(p.title||"") + '</div></td>' +
          '<td>' + esc(p.company) + '</td>' +
          ...
          '<td class="c-actions">' +
            '<a class="btn btn--ghost btn--xs" href="conversations.html?prospect=' + encodeURIComponent(p.id) + '">Outreach</a>' +
          '</td>';
        tbody.appendChild(tr);
      });
      if (!data.items.length) el("prospect-state").innerHTML = '<p class="empty-state">No prospects yet.</p>';
    } catch (e) {
      el("prospect-state").innerHTML = '<p class="empty-state">' + e.message + '</p>';
    }
  }
  ...
  async function loadApprovals() {
    ...
    try {
      ...
      data.items.forEach(function (ap) {
        ...
        li.innerHTML =
          '<div class="approval-card__body">' +
            '<div class="approval-card__title">' + esc(ap.title) + '</div>' +
            '<div class="approval-card__meta">' + esc(ap.prospect_name) + ' · ' + esc(ap.prospect_company) + ' · ' + esc(ap.reason) + ' · ' + reltime(ap.created_at) + '</div>' +
          '</div>' +
          '<div class="approval-card__actions">' +
            '<button class="btn btn--primary btn--sm" data-apr-id="' + ap.id + '" data-action="approve"><svg class="icon"><use href="#i-check"/></svg>Approve</button>' +
            '<button class="btn btn--danger btn--sm" data-apr-id="' + ap.id + '" data-action="reject"><svg class="icon"><use href="#i-x"/></svg>Reject</button>' +
          '</div>';
        list.appendChild(li);
      });
      ...
      el("cd-approvals-state").innerHTML = '<p class="empty-state">' + e.message + '</p>';
    }
  }
  ...
  async function loadAgentRuns() {
    ...
    try {
      ...
      data.items.forEach(function (r) {
        ...
        li.innerHTML =
          '<div class="run-item__icon" aria-hidden="true">' + (statusIcon[r.status]||"·") + '</div>' +
          '<div class="run-item__body">' +
            '<div class="run-item__agent">' + esc(r.agent) + '</div>' +
            '<div class="run-item__summary">' + esc(r.summary) + '</div>' +
            '<div class="run-item__meta">' + reltime(r.started_at) + ' · ' + (r.duration_ms/1000).toFixed(1) + 's</div>' +
          '</div>';
        list.appendChild(li);
      });
      if (!data.items.length) el("runs-state").innerHTML = '<p class="empty-state">No agent runs yet.</p>';
    } catch (e) {
      el("runs-state").innerHTML = '<p class="empty-state">' + e.message + '</p>';
    }
  }
